...
<div class="container my-4">
    ...
    <div class="card shadow-sm">
        <div class="card-header bg-warning">
            <h5 class="mb-0">
                <i class="bi bi-pencil"></i> Chỉnh sửa Biến thể
            </h5>
        </div>
        <div class="card-body">

            <form action="{{ route('admin.bienthe.update', [$sanPham->Ma_San_Pham, $bienThe->Ma_Bien_The]) }}" method="POST">
                @csrf
                @method('PUT')
                            ...
                            <select class="form-select @error('Ma_Size') is-invalid @enderror" 
                                ...
                                @foreach($sizes as $size)
                                    <option value="{{ $size->Ma_Size }}" 
                                            {{ old('Ma_Size', $bienThe->Ma_Size) == $size->Ma_Size ? 'selected' : '' }}>
                                        {{ $size->Ten_Size }}
                                        @if($size->Nhom_Size)
                                            ({{ $size->Nhom_Size }})
                                        @endif
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" 
                                   ...
                                   id="Gia_Ban" 
                                   name="Gia_Ban" 
                                   value="{{ old('Gia_Ban', $bienThe->Gia_Ban) }}"
                                   min="0"
                                   step="1000"
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" 
                                   ...
                                   id="So_Luong_Ton" 
                                   name="So_Luong_Ton" 
                                   value="{{ old('So_Luong_Ton', $bienThe->So_Luong_Ton) }}"
                                   min="0"
                                   placeholder="VD: 100"
                                   required>
                            @error('So_Luong_Ton')
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('Trang_Thai') is-invalid @enderror" 
                                    ...
                                    name="Trang_Thai"
                                    required>
                                <option value="Con_Hang" {{ old('Trang_Thai', $bienThe->Trang_Thai) == 'Con_Hang' ? 'selected' : '' }}>
                                    Còn hàng
                                </option>
                                <option value="Het_Hang" {{ old('Trang_Thai', $bienThe->Trang_Thai) == 'Het_Hang' ? 'selected' : '' }}>
                                    Hết hàng
                                </option>
                            </select>
                            @error('Trang_Thai')
    ...
    <div class="alert alert-info mt-3">
        ...
        <strong>Lưu ý:</strong> Nếu thay đổi Size hoặc Màu sắc, hệ thống sẽ kiểm tra để đảm bảo không trùng lặp với biến thể khác.
    </div>
